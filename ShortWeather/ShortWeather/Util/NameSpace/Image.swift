//
//  Image.swift
//  ShortWeather
//
//  Created by Mac Book Pro on 2022/12/29.
//

import UIKit

public enum Image {

    //MARK: - ShortWeather Logo
    
    public static let logoIcon = UIImage(named: "logo_icon")!
    public static let logoText = UIImage(named: "logo_text")!
    
    //MARK: - Common Icon
    
    public static let icnAdd = UIImage(named: "icn_add")!
    public static let icnCloth = UIImage(named: "icn_cloth")!
    public static let icnDrawer = UIImage(named: "icn_drawer")!
    public static let icnExpandDown = UIImage(named: "icn_expand_down")!
    public static let icnExpandLeft = UIImage(named: "icn_expand_left")!
    public static let icnExpandRight = UIImage(named: "icn_expand_right")!
    public static let icnHamburgermenu = UIImage(named: "icn_hamburgermenu")!
    public static let icnLocation = UIImage(named: "icn_location")!
    public static let icnSetting = UIImage(named: "icn_setting")!
    public static let icnToday = UIImage(named: "icn_today")!
    public static let icnWeek = UIImage(named: "icn_week")!
    
    //MARK: - Weather Icon

    public static let icnClearDay = UIImage(named: "icn_clear_day")!
    public static let icnClearNight = UIImage(named: "icn_clear_night")!
    public static let icnCloudy = UIImage(named: "icn_cloudy")!
    public static let icnDustBad = UIImage(named: "icn_dust_bad")!
    public static let icnDustGood = UIImage(named: "icn_dust_good")!
    public static let icnDustNomal = UIImage(named: "icn_dust_nomal")!
    public static let icnDustWorst = UIImage(named: "icn_dust_worst")!
    public static let icnHeavysnow = UIImage(named: "icn_heavysnow")!
    public static let icnHumidity = UIImage(named: "icn_humidity")!
    public static let icnLightrain = UIImage(named: "icn_lightrain")!
    public static let icnLightsnow = UIImage(named: "icn_lightsnow")!
    public static let icnLotcloudDay = UIImage(named: "icn_lotcloud_day")!
    public static let icnLotcloudNight = UIImage(named: "icn_lotcloud_night")!
    public static let icnPrecipitation1 = UIImage(named: "icn_precipitation_1")!
    public static let icnPrecipitation2 = UIImage(named: "icn_precipitation_2")!
    public static let icnPrecipitation3 = UIImage(named: "icn_precipitation_3")!
    public static let icnPrecipitation4 = UIImage(named: "icn_precipitation_4")!
    public static let icnPrecipitation5 = UIImage(named: "icn_precipitation_5")!
    public static let icnPrecipitation6 = UIImage(named: "icn_precipitation_6")!
    public static let icnPrecipitation7 = UIImage(named: "icn_precipitation_7")!
    public static let icnPrecipitation8 = UIImage(named: "icn_precipitation_8")!
    public static let icnPrecipitation9 = UIImage(named: "icn_precipitation_9")!
    public static let icnPrecipitation10 = UIImage(named: "icn_precipitation_10")!
    public static let icnRain = UIImage(named: "icn_rain")!
    public static let icnRainsnow = UIImage(named: "icn_rainsnow")!
    public static let icnShower = UIImage(named: "icn_shower")!
    public static let icnSnowdrifting = UIImage(named: "icn_snowdrifting")!
    public static let icnSunrise = UIImage(named: "icn_sunrise")!
    public static let icnSunset = UIImage(named: "icn_sunset")!
    
    //MARK: - Weather Image
    
    public static let imgClearDay = UIImage(named: "img_clear_day")!
    public static let imgClearNight = UIImage(named: "img_clear_night")!
    public static let imgCloudy = UIImage(named: "img_cloudy")!
    public static let imgLightrain = UIImage(named: "img_lightrain")!
    public static let imgLightsnow = UIImage(named: "img_lightsnow")!
    public static let imgLotcloudDay = UIImage(named: "img_lotcloud_day")!
    public static let imgLotcloudNight = UIImage(named: "img_lotcloud_night")!
    public static let imgRain = UIImage(named: "img_rain")!
    public static let imgRainsnow = UIImage(named: "img_rainsnow")!
    public static let imgShower = UIImage(named: "img_shower")!
    public static let imgSnow = UIImage(named: "img_snow")!
    public static let imgSnowdrifting = UIImage(named: "img_snowdrifting")!
    
    //MARK: - Background View
    
    public static let backViewCloud = UIImage(named: "img_bg_cloud")!
    public static let backViewDay = UIImage(named: "img_bg_day")!
    public static let backViewFog = UIImage(named: "img_bg_fog")!
    public static let backViewNight = UIImage(named: "img_bg_night")!
    public static let backViewRain = UIImage(named: "img_bg_rain")!
    public static let backViewSnow = UIImage(named: "img_bg_snow")!
    public static let backViewWindy = UIImage(named: "img_bg_windy")!
    public static let backViewYellowdust = UIImage(named: "img_bg_yellowdust")!
}
